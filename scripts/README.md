...
file (described later), and as it's second argument the final compiled
...
visibility for the functions in your library.  It need to generate both
a header and source file.

### Build system integration

You should add targets to your `Makefile.am` or `meson.build` in order
to generate the files and ensure everything is included properly.
...
AM_CONDITIONAL([ENABLE_SYMBOL_VISIBILITY], [test "x$have_gnuc_visibility" = "xyes"])
```

The key points are that, when GNU visibility is enabled, we need to
define `ENABLE_SYMBOL_VISIBILITY` and add `-fvisibility=hidden` to set
the default.
...
If you are not, you will need to change your header guards to match this
pattern (alternatively, you could add support for a
`--header-guard-format` option to the `xdt-gen-visibility` script).
...
rearranges includes, ensure that you've set up a way such that the
visibility header will be sorted last.
...
Of course, replace `THE_FILE_NAME` with the actual upper-snake-case
version of the file's name.  And yes, that's correct: you're including
the `.c` file, not a header, and the `#define` includes the letter "C",
and not "H".
...
Note that it should *not* include the `.c` suffix.
